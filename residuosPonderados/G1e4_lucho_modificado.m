...
%Inicializo
Kg=zeros(2*cant_puntos,2*cant_puntos);
...
for l=1:cant_puntos
    for m=1:cant_puntos
        ...
        %Klm=int(int(transpose(LNl)*D*LNm,x,0,1),y,0,1);
        
        K = sym(zeros(2,2));
        K(1,1) = 8*dNdx(l,1).*dNdx(m,1) + 3*dNdy(l,1).*dNdy(m,1);
        K(1,2) = 2*dNdx(l,1).*dNdy(m,2) + 3*dNdy(l,1).*dNdx(m,2);
        K(2,1) = 2*dNdy(l,2).*dNdx(m,1) + 3*dNdx(l,2).*dNdy(m,1);
        K(2,2) = 8*dNdy(l,2).*dNdy(m,2) + 3*dNdx(l,2).*dNdx(m,2);
        K = int(int(K,x,0,1),y,0,1);
        %Kg(2*l-1:2*l,2*m-1:2*m)=int(int(K,x,0,1),y,0,1);  
        Kg(l,m) = K(1,1);
        Kg(l,m+cant_puntos) = K(1,2);
        Kg(l+cant_puntos,m) = K(2,1);
        Kg(l+cant_puntos,m+cant_puntos) = K(2,2);
    end
    %Para la primera ecuacion vale para la segunda es 0
    if (l <= cant_puntos)
        fg(l) = 6 * int((1-y^2)*subs(N(l,1),x,1),y,0,1);
    end
end
